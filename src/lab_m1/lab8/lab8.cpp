...
void Lab8::Init()
{
    ...
    newlightPosition = glm::vec3(0.5f, 1, 1);
    newlightColor = glm::vec3(0.3f, 0.2f, 0.7f);
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0.7, 0.5));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.4, 0.7, 0.3));
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.5, 0.55, 0.25));
    }
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.6, 0.6, 0.6));
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, newlightPosition);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int isSpot_location = glGetUniformLocation(shader->program, "spot");
    glUniform1i(isSpot_location, spot);

    int new_light_position = glGetUniformLocation(shader->program, "new_light_position");
    glUniform3f(new_light_position, newlightPosition.x, newlightPosition.y, newlightPosition.z);

    int new_light_color = glGetUniformLocation(shader->program, "new_light_color");
    glUniform3f(new_light_color, newlightColor.r, newlightColor.g, newlightColor.b);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_UP)) newlightPosition -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_DOWN)) newlightPosition += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_I)) newlightPosition -= up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_K)) newlightPosition += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_RIGHT)) newlightPosition += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_LEFT)) newlightPosition -= right * deltaTime * speed;
